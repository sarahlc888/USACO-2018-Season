...
public class VisitFJ {
	public static void main(String args[]) throws IOException {
		...
		while (!toVisit.isEmpty()) {
			...
			int curTime = DP[cur.y][cur.x]; // y = row, x = col
			...
			if ((N-1 - cur.x) + (N-1 - cur.y) <= 2) {
				...
				int finalTime = curTime + (N-1 - cur.x)*T + (N-1 - cur.y)*T;
			...
			for (int i = 0; i < nexts.length; i++) {	// loop through & process
				...
				int ogTime = DP[ny][nx]; // old time to reach nexts[i]
				int newTime = curTime + 3*T + grid[ny][nx]; // new time to reach nexts[i]
