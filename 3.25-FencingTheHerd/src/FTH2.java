...
public class FTH2 {
	...
	public static void main(String args[]) throws IOException {
		...
		N = Integer.parseInt(st.nextToken()); // orig num cows
		int Q = Integer.parseInt(st.nextToken()); // num queries
		cows = new ArrayList<Pair>(); 
...
//		System.out.println("hull: " + ch);
		...
		for (int i = 0; i < Q; i++) { // process queries
			...
			if (type == 1) { // add a new cow
				Pair ncow = new Pair(Long.parseLong(st.nextToken()), Long.parseLong(st.nextToken()));
				
				prevch = ch;
				prevch.add( ncow ); // add to cows
				...
				// basecase, add the last cow
				ch.add(prevch.get(prevch.size()-1)); 
				chSet.add(prevch.get(prevch.size()-1)); 
