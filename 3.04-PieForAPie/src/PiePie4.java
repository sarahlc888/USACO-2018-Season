 ...
 * 10/10 test cases when doing a temp fix on leastAbove()
 * 
 * the issue is with the leastAbove() function in the first one
 * 1.5 hour total
 */
public class PiePie4 {
	static int[] dist;
...
//			System.out.println("curPie: " + curPie);
			...
			int ind = leastAbove(ep, curPie.y, 1);
			
//			int ind = 0;
			while (ind < N && ep[ind].y < curPie.y) {
				System.out.println("HERE1 " + ep[ind].y + " vs. " + curPie.y);
				
				ind++;
			}
			
			while (ind < N) {
				if (ep[ind].y > D + curPie.y) break;
				adj.get(N+ind).add(i); // link elsie's jth pie to bessie's ith pie
				ind++;
			}
		}
...
//			System.out.println("curPie: " + curPie);
			...
			int ind = leastAbove(bp, curPie.x, 0);
//			int ind = 0;
			while (ind < N && bp[ind].x < curPie.x) {
				System.out.println("HERE2 " + bp[ind].x + " vs. " + curPie.x);
				ind++;
			}
			while (ind < N) {
				if (bp[ind].x > D + curPie.x) break;
				adj.get(ind).add(i+N);
				ind++;
			}
		}
...
//			System.out.println("lo: " + lo + " hi: " + hi + " mid: " + mid);
			...
			if (ind == 0) 
				...
				curVal = arr[mid].y;
			if (val == 1010) System.out.println("  lo: " + lo + " hi: " + hi + " mid: " + mid + " curVal: " + curVal);
		...
		if (val == 1010) {
			System.out.println(arr[lo].y);
			System.out.println(arr[999].y);
		}
		return lo;
	}
