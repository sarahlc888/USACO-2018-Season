...
public class PiePie3 {
	...
	public static void main(String args[]) throws IOException {
		...
		Pie[] pies1 = new Pie[2*N]; // unsorted pies
		...
		for (int i = 0; i < N; i++) {
			...
			pies1[i] = new Pie(bv, ev, 0, i);// 0 = baked by bessie, i = ID
			pies1[i] = new Pie(bv, ev, 0, i);// 0 = baked by bessie, i = ID
		}
		for (int i = N; i < 2*N; i++) {
			// elsie's pies
			...
			int bv = Integer.parseInt(st.nextToken()); // bessie's rating
			int ev = Integer.parseInt(st.nextToken()); // elsie's rating
			pies1[i] = new Pie(bv, ev, 1, i);// 1 = baked by elsie, i = ID
			pies1[i] = new Pie(bv, ev, 1, i);// 1 = baked by elsie, i = ID
		}
		br.close();
		
		// sort piesB based on Bessie's ratings
		//Arrays.sort(pies1, 0, N, Pie.BessiePieComparator); 
		//Arrays.sort(pies1, N, 2*N, Pie.BessiePieComparator); 
		// sort piesE based on Elsie's ratings
		//Arrays.sort(pies1, 0, N, Pie.ElsiePieComparator); 
		//Arrays.sort(pies1, N, 2*N, Pie.ElsiePieComparator); 
		...
		for (int i = 0; i < N; i++) { // loop through pies baked by bessie
			...
			Pie curPie = pies1[i]; // current pie (baked by bessie)
			...
			if (curPie.eVal != 0) {
				...
				int loInd = bSearch1E(pies1, lbound, 2*N-1, N);
				if (loInd > -1) {
					for (int j = loInd; j < 2*N; j++) {
						if (pies1[j].eVal > hbound) break;
						// add piesE[j].id to adjacency list of curPie.id
						adjs[curPie.id].adj.add(pies1[j].id);
						//System.out.println("  choice: " + piesE[j].id);
						steps[curPie.id][pies1[j].id] = 1;
					}
				}
		...
		for (int i = N; i < 2*N; i++) { // loop through elsie's baked pies (last N pies)
			Pie curPie = pies1[i]; // current pie baked by elsie
			//System.out.println("curPie: " + curPie.id);
			...
			if (curPie.bVal != 0) {
				...
				//System.out.println("  lbound: " + lbound);
				//System.out.println("  hbound: " + hbound);
				int loInd = bSearch1B(pies1, lbound, N-1, 0);
				if (loInd > -1) {
					for (int j = loInd; j < 2*N; j++) {
						if (pies1[j].bVal > hbound) break; // exit condition
						// add piesE[j].id to adjacency list of curPie.id
						adjs[curPie.id].adj.add(pies1[j].id);
		...
		for (int i = 0; i < 2*N; i++) {
			if (pies1[i].bVal == 0 || pies1[i].eVal == 0) {
				zeroes.add(i);
