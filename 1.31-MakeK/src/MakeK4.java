...
// return if you can make K using (+, -, *, /) out of N numbers
// do a fraction class!!! to make everything still be integers
...
public class MakeK4 {
	...
	public static void processOneFile(String filename, int k) throws IOException{
		...
		ops = new int[(int) Math.pow(4, N)][N];
	...
	public static void applyOp(String record, float running, int curInd, int[] curNums) {
		if (curInd == N) {
			//System.out.println("running: " + running);
			// evaluate and end that recursive branch
			if (running == K) {
				// goal achieved
				System.out.println("goal! " + record);
			...
			if (record == "" && running == 0) { // if first time
				applyOp("" + curNums[curInd], (float)curNums[curInd], curInd+1, curNums);
				
			} else {
				applyOp(record +  " + "+curNums[curInd], running + (float) curNums[curInd], curInd+1, curNums);
				applyOp(record +  " - "+curNums[curInd], running - (float) curNums[curInd], curInd+1, curNums);
				applyOp(record +  " * "+curNums[curInd], running * (float) curNums[curInd], curInd+1, curNums);
				applyOp(record +  " / "+curNums[curInd], running / (float) curNums[curInd], curInd+1, curNums);
			}
