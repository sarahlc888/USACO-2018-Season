...
public class FD2 {
	...
	public static void main(String args[]) throws IOException {
		...
		// but since going back from bale N, decrement paths that go out of haybales
//		for (int i = 0; i < K; i++) { 
//			int curHay = hays[i];
...
//		}
		for (int i = 0; i < K; i++) { 
			int curHay = hays[i];
			System.out.println("curHay: " + curHay);
			int ogSize = adj.get(curHay).size();
			for (int j = 0; j < ogSize; j++) {
				System.out.println("j: " + j);
				Pair cur = adj.get(curHay).get(j);
				adj.get(curHay).add(new Pair(cur.weight-tasty[curHay], curHay, true));
			}
		}
