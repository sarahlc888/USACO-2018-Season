...
// delete function deletes multiple at once rather than one at a time - slower though...
...
public class FR4 {
	public static void main(String[] args) throws IOException{
		...
		for (int i = 0; i <= 3; i++) {
			...
			cows1 = deleteCow(0, cows1, 0, i); // kick off i cows
			...
			for (int j = 0; j <= 3-i; j++) {
				...
				cows2 = deleteCow(1, cows2, i, j); // kick off j cows
				...
				for (int k = 0; k <= 3-i-j; k++) {
					...
					cows3 = deleteCow(2, cows3, i+j, k); // kick off k cows
					
					// set m to the highest possible value that will yield the min size
					int m = 3-i-j-k;

					// remove from top
					cows3 = deleteCow(3, cows3, i+j+k, m); // kick off m cows
	...
	public static int[][] deleteCow(int dir, int[][] curcows, int numPriorDeletions, int numDeletions) {
		//int n = 0;
		if (dir == 0) {
			...
			for (int i = 0; i < numDeletions; i++) {
				curcows[numPriorDeletions + i][0] = -1;
				curcows[numPriorDeletions + i][1] = -1;
			}
			...
			*/
			...
			for (int i = 0; i < numDeletions; i++) {
				curcows[curcows.length - 1 - i][0] = -1;
				curcows[curcows.length - 1 - i][1] = -1;
			}
