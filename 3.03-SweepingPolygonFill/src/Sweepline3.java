...
public class Sweepline3 {
	...
	public static void main(String args[]) throws IOException {
		...
		for (int y = minY; y <= maxY; y++) { // LOOP THROUGH ROWS, lo to hi
			...
			minActive = 0;
			System.out.println("  min: " + minActive + "  max: " + maxActive);
			
			ArrayList<Integer> xvals = new ArrayList<Integer>(); // xvals of intersections w/ sweepline
			ArrayList<Pair> horizEdges = new ArrayList<Pair>(); // start and end x vals of horiz edges on sweepline
			for (int j = minActive; j <= maxActive; j++) { // edges
				Line curEdge = edges[j];
				
//				// TODO: fix this
//				if (curEdge.p1.y == y && curEdge.p2.y == y) {
//					horizEdges.add(new Pair(Math.min(curEdge.p1.x, curEdge.p2.x), Math.max(curEdge.p1.x, curEdge.p2.x)));
//					System.out.println("  HORIZ: " + horizEdges.get(horizEdges.size()-1));
//				}
...
//						curEdge.p1.y < y && curEdge.p2.y >= y) { // verify intersection
				if (curEdge.p1.y <= y && curEdge.p2.y >= y) { // verify intersection
					...
					// find intersection
					if (curEdge.p1.y == y && curEdge.p2.y == y) {
						xvals.add(curEdge.p1.x);
						xvals.add(curEdge.p2.x);
						onBounds += 2;
					} else {
						...
						System.out.println("    newx: " + newx + "  nnewx: " + nnewx);
						
						// count if the points are on the edge
						if (newx == nnewx && !xvals.contains(nnewx) || m == 0) {
			...
			while (ind+1 < xvals.size()) {
				...
				if (cur == next) {
					...
					// check if one edge is horizontal
...
//						xvals.remove(ind);
	...
	public static class Line implements Comparable<Line> {
		...
		public int compareTo(Line o) { // sort by p1.y then p2.y, lo to hi
			if (p1.y == o.p1.y) {
				if (p2.y == o.p2.y) return p2.x-o.p2.x;
				return p2.y-o.p2.y;
			}
			return p1.y - o.p1.y;
		}
