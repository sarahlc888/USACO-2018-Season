 ...
 * 
 * 1/10 test cases
 * gave up, will maybe come back
 * 
 */
public class FarmPainting3 {
	public static void main(String args[]) throws IOException {
		...
		for (int x = minx; x <= maxx; x++) {
			...
			while (ind < 2*N && x == corners[ind].x && corners[ind].LL) {
				...
				if (nextInd%2 == 1) { // don't add
					// odd, so it's the end point of another box's yrange
					// it's contained inside something
					
					// make the other one invalid so you won't process it
					// as an end point
					corners[ind].other.x = -1;
					corners[ind].other.y = -1;
