 ...
 * see v2
 * 
 * give smallest bottle of sunscreen to the smallest SPF
 * 
 * not finished at all
 * will not revisit...
 * 
 * idea:
 * greedy for sunscreens, lower bound and upper bound to find cow for it
 * 
 * 
 */
public class Sunscreen {
	public static void main(String args[]) throws IOException {
		...
		TreeSet<Cow> cows1 = new TreeSet<Cow>(a1Comparator); 
		TreeSet<Cow> cows2 = new TreeSet<Cow>(a2Comparator); 
		...
		for (int i = 0; i < C; i++) {
			...
			//lbounds[i] = a;
			cows1.add(new Cow(a, b));
			cows2.add(new Cow(a, b));
		}
		
		PriorityQueue<Bottle> bottles = new PriorityQueue<Bottle>();
		for (int i = 0; i < L; i++) {
			st = new StringTokenizer(scan.nextLine());
			int a = Integer.parseInt(st.nextToken()); // min
			int b = Integer.parseInt(st.nextToken()); // max
			bottles.add(new Bottle(a, b));
		}
		...
		while (!bottles.isEmpty()) {
			// while you still have cows and sunscreen
			Iterator<Cow> c1 = cows1.iterator();
			Iterator<Cow> c2 = cows2.iterator();

			Cow curCow1 = c1.next();
			Cow curCow2 = c2.next();
			
			Bottle curBottle = bottles.peek();
			/*
	...
	public static Comparator<Cow> a1Comparator = new Comparator<Cow>(){
		...
		public int compare(Cow a, Cow c) {
			...
			if (a.maxS < c.maxS) return -1;
			if (a.maxS > c.maxS) return 1;
			if (a.minS < c.minS) return -1;
			if (a.minS > c.minS) return 1;
			return 0;
		}

	};
	public static Comparator<Cow> a2Comparator = new Comparator<Cow>(){
