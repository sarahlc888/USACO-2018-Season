...
public class P3 {
	...
	public static void main(String args[]) throws IOException {
		...
		dist = new long[N]; // distance array from source to i
...
//		System.out.println("paths: " + paths);
		...
		int maxInd = -1;
		for (int i = 1; i < N; i++) {
			long diff = ((long)timesVisited[i])*dist[i] - timesVisited[i]*T; // build a bridge everywhere
...
//		ArrayList<Integer> parent = new ArrayList<Integer>(); // the path so far
		...
		public Pair(long a, int b, int c) { // add something to the path
			weight = a;
			dest = b;
			parent = c;
		}
